...
include(dirname(__FILE__).'/../inc/enteteOld.php');
...
if(isset($_SESSION['authorized']))
{
	 ...
	 $sqlDossier = "SELECT   dossier  from adherentsOld group by dossier ;" ;
...
{
 $sql = "SELECT   ".$elementsFixe.$elements. 'from adherentsOld where categories like "'.$categorie.'"'.$column.';' ;
 $sqlEmail = 'SELECT   email1 ,email2 from adherentsOld where categories like "'.$categorie.'"'.$column.';' ;
}
else if($dossierSelected == "vide")
{
	$sql = "SELECT   ".$elementsFixe.$elements. 'from adherentsOld where dossier IS NULL and categories like "'.$categorie.'"'.$column.';' ;
	$sqlEmail = 'SELECT  email1 ,email2 from adherentsOld where dossier IS NULL and categories like "'.$categorie.'"'.$column.';' ;
  }
else
{
  $sql = "SELECT   ".$elementsFixe.$elements. 'from adherentsOld where dossier = "'.$dossierSelected.'" and categories like "'.$categorie.'"'.$column.';' ;
  $sqlEmail = 'SELECT   email1 ,email2 from adherentsOld where dossier = "'.$dossierSelected.'" and categories like "'.$categorie.'"'.$column.';' ;
}
  //}
//var_dump ($sql);
try {
