...
CREATE TABLE IF NOT EXISTS concours ( id_concours int AUTO_INCREMENT PRIMARY KEY  ,concoursName varchar (100) , concoursDate Date, referent varchar (100) ,note varchar (100), prixAdulte  varchar (5),prixEnfant  varchar (5), CONSTRAINT concourName UNIQUE (concoursName))  ENGINE=MyISAM AUTO_INCREMENT=9 DEFAULT CHARSET=latin1 COLLATE=latin1_general_ci  ;



CREATE TABLE IF NOT EXISTS concoursArchers ( id_concoursArchers  int AUTO_INCREMENT PRIMARY KEY ,id_adherent int ,id_concours int,blason varchar (30), depart VARCHAR (10) ,ConcoursPrix float , etat  varchar (5), CONSTRAINT uniqueNom UNIQUE (id_adherent, id_concours),  FOREIGN KEY (id_concours) REFERENCES concours (id_concours) ON DELETE CASCADE, FOREIGN KEY (id_adherent)  REFERENCES adherents (id_adherent) ON DELETE CASCADE ) ENGINE=MyISAM AUTO_INCREMENT=9 DEFAULT CHARSET=latin1 COLLATE=latin1_general_ci;
